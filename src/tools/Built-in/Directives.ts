...
function Directives2Md(element: TElement): vscode.MarkdownString {
    const {
        keyRawName,
        doc,
        link,
...
type TSnippetDirectivee = readonly vscode.CompletionItem[];

export const [SnippetDirectives, DirectivesMDMap] = ((): [TSnippetDirectivee, TDirectivesMDMap] => {
    //
    ...
    for (const [k, v] of Object.entries(DirectivesList)) {
        const md: vscode.MarkdownString = Directives2Md(v);
        map2.set(k, md);
