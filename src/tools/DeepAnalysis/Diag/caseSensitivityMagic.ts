...
export function ParseDiagCaseMsg(diag: vscode.Diagnostic): TParseDiagCaseMsg {
    ...
    const characterExec: RegExpExecArray | null = (/(\d+)\]/ui).exec(message);
    // eslint-disable-next-line no-magic-numbers
    if (magStrList.length !== 2 || lineExec === null || characterExec === null) {
        throw new Error(`ParseDiagCaseMsg Err--59--12--14,diag is ${diag}`);
    }

    const radix = 10;
    const defLine: number = parseInt(lineExec[1], radix) - 1;
    const defChar: number = parseInt(characterExec[1], radix) - 1;
