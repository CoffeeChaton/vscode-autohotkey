...
function setValMap(uri: vscode.Uri, ahkSymbol: TAhkSymbol, DocStrMap: TTokenStream): TValMap {
    const valMap: TValMap = getValDef(uri, ahkSymbol, DocStrMap);
...
export function DeepAnalysis(document: vscode.TextDocument, ahkSymbol: TAhkSymbol): null | DeepAnalysisResult {
    ...
    const valMap: TValMap = setValMap(uri, ahkSymbol, DocStrMap);
